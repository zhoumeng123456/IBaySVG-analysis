...
#' result <- NBIMSVG_noinflation(spelist = spelist,c_alpha = c_alpha,num_cores = 1)
#'
#' # Print key outputs:
...
#' print(result[[4]])
#' }
NBIMSVG_noinflation <- function(spelist,c_alpha=NULL,num_cores=1,acrate=0.01,alpha=0.05,gamma1=sqrt(0.001),gamma2=0.01,min_iter=30,max_iter=150,speci_iter=12,
  ...
  clusterExport(cl, c("sim_create", "CTIG", "f", "gausslq", "compu_phi",
                      ...
                      "VIZINB_noinflation", "citgtest_noinflation", "c_alpha", "spelist","tun_spl"))
  ...
  for (i in seq(1, length(input_data), by = chunk_size)) {
    ...
    result_chunk <- parLapply(cl, chunk_data, function(x, params,use_covariate) {
      ...
      result <- tryCatch({
        withTimeout({
            citgtest_noinflation(x, result = result_ctig, c_alpha = c_alpha, spelist = spelist,
...
#'
#'
VIZINB_noinflation <- function(y,splinevalue,samplenumber,splinelevel,acrate=0.01,ak=1,dp=1.8,cp=0.2,c_alpha,knum=6,cq=1,dq=1,
...
#' @export
citgtest_noinflation <- function(g,result,c_alpha,spelist,acrate,gamma1,gamma2,min_iter,max_iter,speci_iter,use_covariate=TRUE,
  ...
  bbb <- VIZINB_noinflation(Ysingle.list,result[[2]][[tun_choose]],result[[3]],tun_choose,acrate=acrate,ak=ak,dp=dp,cp=cp,cq=cq,dq=dq,c_alpha=c_alpha,knum=knum1,
